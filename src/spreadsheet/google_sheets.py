...
class GoogleSheetsConnector:
    ...
    def connect(self, read_only: bool = False) -> None:
        ...
        Args:
            read_only: When True, skips auto-creating missing output header columns.
                       Use this in the prototype/testing context to avoid touching the sheet.
        """
        ...
        try:
            ...
            # Map column names; read_only=True skips creating missing output columns
    ...
    def _map_and_initialize_headers(self, read_only: bool = False) -> None:
        ...
        for key, aliases in self.expected_outputs.items():
            ...
            if idx:
                ...
                if read_only:
                    # In read-only mode: don't attempt to create columns, just skip mapping
                    logger.debug(f"read_only mode: skipping missing output column '{aliases[0]}'")
                else:
                    # Production mode: plan to append the missing header to the sheet
    ...
    def read_scholarships(self) -> List[Dict[str, Any]]:
        ...
        This extracts the visible text AND the underlying hyperlink URLs from cells.
        """
        if not self.wks:
        ...
        try:
            # Query Sheets API spreadsheets.get for RowData including userEnteredValue and hyperlink
            range_name = f"'{self.wks.title}'!A1:T500"  # Read up to column T, row 500
            ...
            for idx, r in enumerate(data_rows, start=2): # Data starts at row 2
                ...
                # Parse historical method or default to Online
            ...
            logger.info(f"Loaded {len(scholarships)} active scholarship records from sheet with hyperlinks.")
            return scholarships
    ...
    def batch_write_results(self, verification_results: List[Dict[str, Any]]) -> None:
        ...
        for res in verification_results:
            ...
            # Map Pydantic verification fields to sheet columns
            # Formats links as Google Sheets HYPERLINK formulas to match the user's style!
